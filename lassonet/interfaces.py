...
class AdaptiveLassoNet(BaseLassoNet, ABC):
    def __init__(self, penalty_type='scad'):
        super().__init__()
        ...
        *,
        ...
        def validation_obj():
            with torch.no_grad():
                return (
                    self.criterion(model(X_val), y_val).item()
                    + lambda_ * model.l1_regularization_weighted_skip().item()
                    + self.gamma * model.l2_regularization().item()
                    + self.gamma_skip * model.l2_regularization_skip().item()
                )
        ...
        if self.backtrack:
            ...
            n_iters = epoch + 1
        with torch.no_grad():
            reg = self.model.l1_regularization_weighted_skip().item()
            l2_regularization = self.model.l2_regularization()
            l2_regularization_skip = self.model.l2_regularization_skip()
        return HistoryItem(
            lambda_=lambda_,
    ...
    def path(
        ...
        *,
        ...
        for current_lambda in itertools.chain([lambda_start], lambda_seq):
            ...
            self.penalty_weights = last.state_dict["skip.weight"]
            for j in range(self.penalty_weights.shape[1]):
                self.penalty_weights[0][j] = self.scad_penalty(self.penalty_weights[0][j], current_lambda)
